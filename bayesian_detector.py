...
class BayesianDetector:
  ...
  def train_best_detector(
    ...
    """
    # Fixed inverted logic from reference: raise if NOT cuda or tpu
    if torch_device.type not in ("cuda", "tpu", "mps"):
        # Warning instead of error for now, as user might want to run on CPU for testing
        print("Warning: Training might be unstable on CPU. Use GPU or TPU for best results.")
